...
def train(epoch,args):
    ...
    while True:
        ...
        loss = (-lossAdv)  + lossSize/100 + lossCompact/100000
        ...
        if np.random.choice([0,1], 1, p = [1 - args.prob, args.prob])[0] == 1:
            ...
            maskedFeatures[maskedFeatures == 0] = torch.Tensor([random.uniform(-1,1 ) for _ in range(len(maskedFeatures[maskedFeatures == 0]))]).cuda()
...
if args.checkpoint == -1:
    ...
    if args.startfolder == -1:
        ...
        maskNet.load_state_dict(torch.load('saved_models_ce_masked'+ str(args.startfolder) + '/maskNet_' + str(args.checkpoint) + '.pth'))
